        ...
        var
            ...
            columnDefs = [{
...

                {
                    ...
                    render: function(data, type, full, meta) {
                        var id = full.id;
                        return '<a title="Edit Record" href="javascript: void(0);" onclick="openEditModal(' + id +
                            ...
                            ' <a title="Delete Record" href="javascript: void(0);" onclick="removeData(' + full.id +
                            ')"class="delete svg-bg" style="background-color: rgba(236,0,0,0.07); color:#EC0000"><i class="fa-solid fa-trash-can" style="font-size: 14px;"></i>';
                    }
                }
            ],
            dataTable = callDataTable('data-datatable', '<?php echo route('admin-users.data'); ?>', filters, columns, '', '',
                columnDefs);
    ...
    $(document).on('change', '.switch-input ', function() {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    url: "<?php echo e(route('admin-users.update-status')); ?>",
                    type: 'GET',
                    data: {
                        'status': status,
                        'id': id
...
<?php /**PATH /home/dataseed/prod-thomascook.dataseedtech.com/resources/views/stacks/js/modules/admin-user/index.blade.php ENDPATH**/ ?>